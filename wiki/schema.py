        ...
        for b in (wk.get("sections", {}).get("basics") or []):
            ...
            for row in b.get("rows") or []:
                ...
                if (not no or len(no) > 30 or not re.search(r"\d", no)
                    ...
                    continue
                # **键名含「藏品号」不等于值就是藏品号。**扩大键名匹配后立刻撞上:
                # 「藏品号系别」是描述字段（值形如「「故」= 1925 年建院时的旧藏」），
                # 键里带「藏品号」三字就被当成标识符读了；两条写同样的系别说明，
                # 就被判成同号重复。长度与忌用词两道闸都没拦住——那句话不足 30 字，
                # 且含数字 1925。
                # 判据改用**值的形态**而非键名: 藏品号里不会出现中文标点或空格。
                # 这比继续往忌用词表里加词可靠——词表要穷举，标点是形态特征。
                if re.search(r"[，。；、「」『』（）\s=＝]", no):
                    continue
