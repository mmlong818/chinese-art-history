...
另有三家经 2026-08-04 复测确认可编程取到，尚未接入（详见 MUSEUMS.md）：
    芝加哥艺术博物馆 api.artic.edu       provenance_text／exhibition_history，无需 key
    史密森尼国立亚洲艺术博物馆 api.si.edu 递藏逐条带日期与档案编号，全 CC0，须申请 key
...
它是该馆自己网页在用的公开接口，读它等同于读其公开页面；但**无文档、随时可变**，
不可当作稳定依赖。

台北故宫原写作「SSL 握手超时」，**也是错的**：实测是证书链验证失败
（服务端未下发中间证书），不是握手不上。这个区别要紧，因为两者的解法不同。
**本库不采用关闭证书校验的做法**——那会同时放弃对中间人的防护，
为了一份年代数据不值得。正确解法是补齐中间证书链再验证；未补齐之前，
台北故宫一律按人工核对处理。上海博物馆实测连接失败数次，可能是暂时性的，
不作永久结论，留待复测。
