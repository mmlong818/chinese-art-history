...
# —— 藏品号前缀是一条递藏线索，也是一个陷阱 ——
#
# **实测**（80,600 个去重藏品号，2026-08-08 扫至碑帖带后重算；
# 旧数据基于 36,400 条样本，各系计数与「资」系的构成均已过时）:
#
#   新   45,392   1949 年以后征集、调拨、捐赠入藏
#   故   30,707   1925 年建院时接收的清宫旧藏
#   书    2,015   **古籍文献**——该系 100% 为 Historical Documents
#                （如《晚晴簃诗汇》逐页著录）。不属书画，出本库范围。
#   标    1,164   **标本**——该系 100% 为瓷片。研究标本，非完整器。
#   资    1,322   见下。旧记 232 条且称「全为绘画与法书」，是样本未及碑帖带所致。
#
# **「资」不是第六系，是加在别系之上的限定词。**这是本次扫描才看清的结构:
#
#   资拓    958   Rubbings          资玺册  130   Imperial Seals
#   资绘画   127   Paintings         资新    102   Paintings
#   资书法     2   资青铜／资甲骨／资织杂 各 1
#
# 判据有两条，都不依赖猜前缀的字面意思:
#   1. **「资」后面跟的是门类词**（拓／玺册／绘画／书法／青铜／甲骨／织杂），
#      而「故」「新」两系号内从不含非数字段（抽 400 条，0 例）。
#   2. **「资新00092670」把「资」加在了「新」号之上**——限定词套在既有系列外面，
#      这是「资」与故／新不同层级的直接证据。
#
# 含义仍未经馆方确认。「资料」是最自然的读法（同「书＝书籍、标＝标本」的构词），
# 而这批记录的旁证也都指向它: 年代字段 955/1322 为空、hasImage 1090/1322 为 0、
# 题名多为「李成寒林图轴」「钱选三多图轴」一类挂名之作而非该家的名件。
# **但旁证不是确认，故立目禁令原样保留**——理由现在更硬:
#   「资绘画00000115 李成寒林图轴」与本库 li-cheng-hanlin-tu（台北故宫
#   故畫000034N000000000「宋李成寒林圖　軸」）同题而分属两馆两号，
#   若把「资」号当原作号回填，等于把两件不同的物合成一件。
# **故本库对「资」字号一律不立目、不回填，待向馆方或专业目录核实后再定。**
#
...
# 下面这条正则覆盖各系，供对账使用。
#
# **同一个坑第二次**: 上一版只列了 资绘画／资书法／资甲骨／资新 四种门类词，
# 而扫到碑帖带后才知另有 资拓 958、资玺册 130、资青铜／资织杂 各 1——
# 按 80,600 号实测，旧正则漏匹 1,090 号，且漏的全在「资」系。
# 教训与上一版相同: **门类词不能靠已见样本枚举**，它取决于扫过哪些带；
# 故此处改为「资」后允许任意 1–2 个非数字字符，再以实测复核漏匹为 0。
ACC_RE = r"(?:资[^\d]{0,2}|故|新|书|書|标|標)\d{5,8}(?:-\d+/\d+)?"
#
